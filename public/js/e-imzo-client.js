...
String.prototype.splitKeep = function(splitter, ahead) {
    ...
    if (splitter != '') {
        function getSubst(value) {
            ...
            var subst = '';
            for (var i = 0; i < value.length; i++) { subst += substChar; }
            return subst;
        };
        var matches = [];
        var replaceName = splitter instanceof RegExp ? "replace" : "replaceAll";
        var r = self[replaceName](splitter, function(m, i, e) { matches.push({value: m, index: i}); return getSubst(m); });
        var lastIndex = 0;
        for (var i = 0; i < matches.length; i++) {
            var m = matches[i];
            var nextIndex = ahead == true ? m.index : m.index + m.value.length;
            if (nextIndex != lastIndex) { var part = self.substring(lastIndex, nextIndex); result.push(part); lastIndex = nextIndex; }
        };
        if (lastIndex < self.length) { var part = self.substring(lastIndex, self.length); result.push(part); };
...
var EIMZOClient = {
        ...
        if (!EIMZOClient.NEW_API) {
            ...
            if (EIMZOClient.NEW_API2) {
                EIMZOClient._findPfxs2(itemIdGen, itemUiGen, items, errors, function(firstItmId2) {
                    ...
                    else { success(items, items.length === 1 ? firstItmId2 : null); }
                });
            } else {
                EIMZOClient._findPfxs2(itemIdGen, itemUiGen, items, errors, function(firstItmId2) {
                    EIMZOClient._findTokens2(itemIdGen, itemUiGen, items, errors, function(firstItmId3) {
                        if (items.length === 0 && errors.length > 0) { fail(errors[0].e, errors[0].r); }
                        else { success(items, items.length === 1 ? (firstItmId2 || firstItmId3) : null); }
                    });
                });
            }
        }
                    ...
                    if (data.success) {
                        ...
                        if (verifyPassword) {
                            CAPIWS.callFunction({name: "verify_password", plugin: "pfx", arguments: [id]}, function(event, data) {
                                if (data.success) { success(id); } else { fail(null, data.reason); }
                            }, function(e) { fail(e, null); });
                        } else { success(id); }
                    } else { fail(null, data.reason); }
                }, function(e) { fail(e, null); });
    ...
    createPkcs7: function(id, data, timestamper, success, fail, detached, isDataBase64Encoded) {
        var data64;
        if (isDataBase64Encoded === true) { data64 = data; } else { data64 = Base64.encode(data); }
        if (detached === true) { detached = 'yes'; } else { detached = 'no'; }
        CAPIWS.callFunction({plugin: "pkcs7", name: "create_pkcs7", arguments: [data64, id, detached]}, function(event, data) {
            if (data.success) { success(data.pkcs7_64); } else { fail(null, data.reason); }
        }, function(e) { fail(e, null); });
    },
    ...
    _getX500Val: function(s, f) {
        ...
        for (var i in res) {
            ...
            if (n !== -1) { return res[i].slice(n + f.length + 1 + (i > 0 ? 1 : 0)); }
        }
        return "";
    },
        ...
        CAPIWS.callFunction({plugin: "pfx", name: "list_all_certificates"}, function(event, data) {
            if (data.success) {
                for (var rec in data.certificates) {
                    ...
                    var vo = {
                        disk: el.disk, path: el.path, name: el.name, alias: el.alias,
            ...
            callback(itmkey0);
        }, function(e) { errors.push({e: e}); callback(itmkey0); });
    },
        ...
        CAPIWS.callFunction({plugin: "ftjc", name: "list_all_keys", arguments: ['']}, function(event, data) {
            if (data.success) {
                for (var rec in data.tokens) {
                    ...
                    x500name_ex = x500name_ex.replace("1.2.860.3.16.1.2=", "PINFL=");
                    var vo = {
                        cardUID: el.cardUID, statusInfo: el.statusInfo, ownerName: el.ownerName, info: el.info,
                        serialNumber: EIMZOClient._getX500Val(x500name_ex, "SERIALNUMBER"),
